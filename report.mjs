import { BUDGET, DIRECTIONS, EVENTS } from "./data.mjs";
import { getDistrict, getMeasure } from "./engine.mjs";
...
export function buildSlides({ result, analysis, space, rank, stress, robust, teamName, aiText }) {
  ...
  slides.push(`<section class="slide"><p class="eyebrow">01 · ПЯТЬ РЕШЕНИЙ</p><h3>Что мы сделали и что это дало</h3><table><thead><tr><th>Мера</th><th>Где</th><th>Стоимость</th><th>Вклад в Score</th></tr></thead><tbody>${decisions.map((decision) => {
    ...
    return `<tr><td><b>${measure.id}</b> ${esc(measure.name)}<small>${DIRECTIONS.find((d) => d.id === measure.direction).label}</small></td><td>${decision.districtId ? getDistrict(decision.districtId).name : "весь город"}</td><td>${measure.cost} ед.</td><td><b>${signed(decision.contribution)}</b></td></tr>`;
  }).join("")}</tbody></table>${result.synergies.length ? `<p>Синергии: ${result.synergies.map((s) => `${s.measures.join(" + ")} (+${s.bonus} ${s.indicator})`).join(", ")}.</p>` : ""}</section>`);
  ...
  slides.push(`<section class="slide"><p class="eyebrow">04 · УСТОЙЧИВОСТЬ И ДАЛЬШЕ</p><h3>Что может пойти не так и как улучшить</h3><ul><li>Устойчивость: в 80% симуляций (неточные эффекты, задержки запуска) Score от ${fmt(robust.p10)} до ${fmt(robust.p90)}.</li><li>Стресс-тест: покрыто ${stress.filter((s) => s.covered).length} из ${stress.length} событий${worst.loss > 0 ? `; худшее — «${esc(EVENTS.find((e) => e.id === worst.eventId).name)}» (−${fmt(worst.loss)} п.)` : ""}.</li>${space ? `<li>Оптимум модели — ${fmt(space.best.score)} за ${space.best.cost} ед.; экономный вариант — ${fmt(space.alternatives.economical.score)} за ${space.alternatives.economical.cost} ед.</li>` : ""}${analysis.tradeoffs.slice(0, 1).map((item) => `<li>${esc(item)}</li>`).join("")}</ul><p class="slide-note">Модель и данные синтетические. Расчёт проверен формально (Z3, Lean 4 + Mathlib).</p></section>`);
