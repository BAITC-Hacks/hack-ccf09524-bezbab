...
import { BASE_CELLS, K, N_DISTRICTS, SHARES, WEIGHTS, effectCells, measureIndex, scoreOf, stressTest, validate } from "./engine.mjs";

// Точный оптимизатор. Пространство: C(14,5) наборов мер × до 5^5 вариантов районов ≈ 1,4 млн комбинаций.
...
const COST = Int32Array.from(MEASURES, (measure) => measure.cost);
const DIRECTION = Int32Array.from(MEASURES, (measure) => DIRECTIONS.findIndex((direction) => direction.id === measure.direction));
const IS_CITY = MEASURES.map((measure) => measure.scope === "city");
const pack = (list) => ({ idx: Int32Array.from(list, ([cell]) => cell), val: Float64Array.from(list, ([, value]) => value) });
const EFFECTS = MEASURES.map((measure) => (measure.scope === "city" ? [pack(effectCells(measure, 0))] : DISTRICTS.map((_, d) => pack(effectCells(measure, d)))));
const indicatorIndex = Object.fromEntries(INDICATORS.map((indicator, index) => [indicator.id, index]));
const SYN = SYNERGIES.map((synergy) => {
...
const GLOBAL_CONFLICTS = CONFLICTS.filter((conflict) => !conflict.sameDistrict).map((conflict) => conflict.measures.map((id) => measureIndex[id]));
const DISTRICT_CONFLICTS = CONFLICTS.filter((conflict) => conflict.sameDistrict).map((conflict) => conflict.measures.map((id) => measureIndex[id]));
const THRESHOLD = SCORE_RULES.criticalThreshold;
...
// visit(score, cost, sel, dist, weakest) вызывается для каждого допустимого набора.
...
export function enumerate(visit, { budget = BUDGET } = {}) {
  ...
  function leaf(cost) {
    ...
    visited += 1;
    visit(SCORE_RULES.averageWeight * average + SCORE_RULES.weakestWeight * weakest - SCORE_RULES.criticalPenalty * critical, cost, sel, dist, weakest);
  }
...
export function analyzeSpace({ topSize = 2000 } = {}) {
  ...
  const visited = enumerate((score, cost, sel, dist, weakest) => {
  ...
  for (let i = 0; i < count; i += 1) histogram[Math.min(bins - 1, Math.floor((sorted[i] - min) / (max - min) * bins))] += 1;
...
export function bestSingleChange(decisions) {
...
export function reallocate(decisions, eventId) {
  ...
  const limit = BUDGET - event.responseCost;
  ...
  enumerate((score, cost, sel, dist) => {
    ...
    if (!kept || score > kept.score + 1e-9 || (Math.abs(score - kept.score) <= 1e-9 && cost < kept.cost)) byChanges[changes] = { score, cost, changes, decisions: toDecisions(sel, dist) };
  }, { budget: limit });
