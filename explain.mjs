import { BUDGET, DIRECTIONS, DISTRICTS, EVENTS, HORIZON, INDICATORS, SCORE_RULES } from "./data.mjs";
...
const label = (decision) => `${decision.measureId} «${getMeasure(decision.measureId).name}» ${place(decision)}`;
const cellName = (item) => `${item.indicator} (${lowerFirst(getIndicator(item.indicator).name)}) в районе ${getDistrict(item.districtId).name}`;
...
export function analyze(result, { stress = [], robust = null, space = null, rank = null } = {}) {
  ...
  for (const synergy of result.synergies) strengths.push(`Сработала синергия ${synergy.measures.join(" + ")}: +${synergy.bonus} к ${synergy.indicator} ${synergy.districtId ? `в районе ${getDistrict(synergy.districtId).name}` : "по городу"} (${synergy.note}).`);
  ...
  if (slow.length) risks.push(`Долгий запуск: ${slow.map((decision) => `${decision.measureId} (лаг ${getMeasure(decision.measureId).lag} кв., за ${HORIZON / 4} года — ${Math.round(realizedShare(getMeasure(decision.measureId)) * 100)}% эффекта)`).join(", ")}. Часть результата придёт уже после горизонта симуляции.`);
  ...
  for (const decision of negative) {
    ...
    risks.push(`Побочный эффект: ${label(decision)} снижает ${indicator} (${lowerFirst(getIndicator(indicator).name)}) на ${fmt(Math.abs(value * realizedShare(getMeasure(decision.measureId))))} — школьные зоны замедляют трафик.`);
  }
  ...
  for (const { district, delta } of changes) {
    ...
    const moved = INDICATORS.map((indicator) => ({ indicator, delta: district.metrics[indicator.id] - before[district.id].metrics[indicator.id] }))
      ...
      .map((item) => `${lowerFirst(item.indicator.name)} ${signed(item.delta)}`);
  ...
  if (space) {
    ...
    else tradeoffs.push("Сценарий совпадает с глобальным максимумом модели (он доказан полным перебором и Z3).");
  }
...
export function explanationFacts(result, { stress = [], robust = null, space = null, rank = null } = {}) {
  return {
    rules: "Score = 0.7 × средняя по населению оценка районов + 0.3 × оценка самого слабого района − 1 × число показателей ниже 40.",
    score: result.score,
    ...
    synergies: result.synergies.map((synergy) => ({ pair: synergy.measures.join("+"), indicator: synergy.indicator, bonus: synergy.bonus, district: synergy.districtId ? getDistrict(synergy.districtId).name : "город" })),
    ...
    criticalBefore: result.baselineCritical.map((item) => ({ district: getDistrict(item.districtId).name, indicator: item.indicator, value: item.value })),
    criticalAfter: result.critical.map((item) => ({ district: getDistrict(item.districtId).name, indicator: item.indicator, value: item.value })),
